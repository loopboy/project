...
def add_train_args(parser):
    ...
    files.add_argument('--train_sub', nargs='+', type=str,
                       help='Preprocessed train source subtoken file')
    ...
    files.add_argument('--train_ky', nargs='+', type=str,
                       help='Preprocessed train code keyword')
    files.add_argument('--train_it', nargs='+', type=str,
                       help='Preprocessed train intoken')
    files.add_argument('--train_is', nargs='+', type=str,
                       help='Preprocessed train instatement')
    files.add_argument('--train_df', nargs='+', type=str,
                       help='Preprocessed train dataflow')
    files.add_argument('--train_cf', nargs='+', type=str,
                       help='Preprocessed train controlflow')
    files.add_argument('--dev_src', nargs='+', type=str, required=True,
                       help='Preprocessed dev source file')
    files.add_argument('--dev_sub', nargs='+', type=str,
                       help='Preprocessed dev source subtoken file')
    ...
    files.add_argument('--dev_ky', nargs='+', type=str,
                       help='Preprocessed dev code keyword')
    files.add_argument('--dev_it', nargs='+', type=str,
                       help='Preprocessed dev intoken')
    files.add_argument('--dev_is', nargs='+', type=str,
                       help='Preprocessed dev instatement')
    files.add_argument('--dev_df', nargs='+', type=str,
                       help='Preprocessed dev dataflow')
    files.add_argument('--dev_cf', nargs='+', type=str,
                       help='Preprocessed dev controlflow')
...
def set_defaults(args):
    ...
    if not args.only_test:
        ...
        args.train_src_subtoken_files = []
        args.train_src_tag_files = []
        args.train_src_keyword_files = []
        args.train_src_intoken_files = []
        args.train_src_instatement_files = []
        args.train_src_dataflow_files = []
        args.train_src_controlflow_files = []  
        ...
        if num_dataset > 1:
            ...
            if len(args.train_sub) == 1:
                args.train_sub = args.train_sub * num_dataset
            ...
            if len(args.train_ky) == 1:
                args.train_ky = args.train_ky * num_dataset
            if len(args.train_it) == 1:
                args.train_it = args.train_it * num_dataset
            if len(args.train_is) == 1:
                args.train_is = args.train_is * num_dataset
            if len(args.train_df) == 1:
                args.train_df = args.train_df * num_dataset
            if len(args.train_cf) == 1:
                args.train_cf = args.train_cf * num_dataset
        ...
        for i in range(num_dataset):
            ...
            train_sub = os.path.join(data_dir, args.train_sub[i])
            train_tgt = os.path.join(data_dir, args.train_tgt[i])
            train_ky = os.path.join(data_dir, args.train_ky[i])
            train_it = os.path.join(data_dir, args.train_it[i])
            train_is = os.path.join(data_dir, args.train_is[i])
            train_df = os.path.join(data_dir, args.train_df[i])
            train_cf = os.path.join(data_dir, args.train_cf[i])
            if not os.path.isfile(train_src):
                raise IOError('No such file: %s' % train_src)
            if not os.path.isfile(train_sub):
                raise IOError('No such file: %s' % train_sub)
            if not os.path.isfile(train_tgt):
                raise IOError('No such file: %s' % train_tgt)
            if not os.path.isfile(train_ky):
                raise IOError('No such file: %s' % train_ky)
            if not os.path.isfile(train_it):
                raise IOError('No such file: %s' % train_it)
            if not os.path.isfile(train_is):
                raise IOError('No such file: %s' % train_is)
            if not os.path.isfile(train_df):
                raise IOError('No such file: %s' % train_df)
            if not os.path.isfile(train_cf):
                raise IOError('No such file: %s' % train_cf)
            ...
            args.train_src_subtoken_files.append(train_sub)
            args.train_tgt_files.append(train_tgt)
            args.train_src_keyword_files.append(train_ky)
            args.train_src_intoken_files.append(train_it)
            args.train_src_instatement_files.append(train_is)
            args.train_src_dataflow_files.append(train_df)
            args.train_src_controlflow_files.append(train_cf)
            args.train_src_tag_files.append(train_src_tag)

    args.dev_src_files = []
    args.dev_src_subtoken_files = []
    args.dev_src_tag_files = []
    args.dev_src_keyword_files = []
    args.dev_src_intoken_files = []
    args.dev_src_instatement_files = []
    args.dev_src_dataflow_files = []
    args.dev_src_controlflow_files = []  
    args.dev_tgt_files = []
    ...
    if num_dataset > 1:
        ...
        if len(args.dev_sub) == 1:
            args.dev_sub = args.dev_sub * num_dataset
        ...
        if len(args.dev_ky) == 1:
            args.dev_ky = args.dev_ky * num_dataset
        if len(args.dev_it) == 1:
            args.dev_it = args.dev_it * num_dataset
        if len(args.dev_is) == 1:
            args.dev_is = args.dev_is * num_dataset
        if len(args.dev_df) == 1:
            args.dev_df = args.dev_df * num_dataset
        if len(args.dev_cf) == 1:
            args.dev_cf = args.dev_cf * num_dataset
    ...
    for i in range(num_dataset):
        ...
        dev_sub = os.path.join(data_dir, args.dev_sub[i])
        dev_tgt = os.path.join(data_dir, args.dev_tgt[i])
        dev_ky = os.path.join(data_dir, args.dev_ky[i])
        dev_it = os.path.join(data_dir, args.dev_it[i])
        dev_is = os.path.join(data_dir, args.dev_is[i])
        dev_df = os.path.join(data_dir, args.dev_df[i])
        dev_cf = os.path.join(data_dir, args.dev_cf[i])
        if not os.path.isfile(dev_src):
            raise IOError('No such file: %s' % dev_src)
        if not os.path.isfile(dev_sub):
            raise IOError('No such file: %s' % dev_sub)
        if not os.path.isfile(dev_tgt):
            raise IOError('No such file: %s' % dev_tgt)
        if not os.path.isfile(dev_ky):
            raise IOError('No such file: %s' % dev_ky)
        if not os.path.isfile(dev_it):
            raise IOError('No such file: %s' % dev_it)
        if not os.path.isfile(dev_is):
            raise IOError('No such file: %s' % dev_is)
        if not os.path.isfile(dev_df):
            raise IOError('No such file: %s' % dev_df)
        if not os.path.isfile(dev_cf):
            raise IOError('No such file: %s' % dev_cf)
        if args.use_code_type:
        ...
        args.dev_src_subtoken_files.append(dev_sub)
        args.dev_tgt_files.append(dev_tgt)
        args.dev_src_keyword_files.append(dev_ky)
        args.dev_src_intoken_files.append(dev_it)
        args.dev_src_instatement_files.append(dev_is)
        args.dev_src_dataflow_files.append(dev_df)
        args.dev_src_controlflow_files.append(dev_cf) 
...
def main(args):
    ...
    # args.train_src_tag_file指的是一些辅助信息如type等，默认设为了false
    train_exs = []
    if not args.only_test:
        args.dataset_weights = dict()
        for train_src, train_sub, train_src_tag, train_ky, train_it, train_is, train_df, train_cf, train_tgt, dataset_name in \
                zip(args.train_src_files, args.train_src_subtoken_files, args.train_src_tag_files, 
                    args.train_src_keyword_files, args.train_src_intoken_files, args.train_src_instatement_files, 
                    args.train_src_dataflow_files, args.train_src_controlflow_files, 
                    args.train_tgt_files, args.dataset_name):
            train_files = dict()
            train_files['src'] = train_src
            train_files['src_sub'] = train_sub
            train_files['src_tag'] = train_src_tag
            train_files['src_keyword'] = train_ky
            train_files['src_intoken'] = train_it
            train_files['src_instatement'] = train_is
            train_files['src_dataflow'] = train_df
            train_files['src_controlflow'] = train_cf
    ...
    dev_exs = []
    for dev_src, dev_sub, dev_src_tag, dev_ky, dev_it, dev_is, dev_df, dev_cf, dev_tgt, dataset_name in \
            zip(args.dev_src_files, args.dev_src_subtoken_files, args.dev_src_tag_files, 
                args.dev_src_keyword_files, args.dev_src_intoken_files,args.dev_src_instatement_files, 
                args.dev_src_dataflow_files, args.dev_src_controlflow_files, 
                args.dev_tgt_files, args.dataset_name):
        dev_files = dict()
        dev_files['src'] = dev_src
        dev_files['src_sub'] = dev_sub
        dev_files['src_tag'] = dev_src_tag
        dev_files['src_keyword'] = dev_ky
        dev_files['src_intoken'] = dev_it
        dev_files['src_instatement'] = dev_is
        dev_files['src_dataflow'] = dev_df
        dev_files['src_controlflow'] = dev_cf
