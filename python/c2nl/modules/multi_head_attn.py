...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, key, value, query,code_keyword_rep=None,code_intoken_map=None,code_instatement_map=None,code_dataflow=None,
                code_controlflow=None,heads_type=None,mask=None, layer_cache=None,attn_type=None, step=None, coverage=None):
        """
        ...
        # prepare keyword,intoken,instatement maps
        ...
        if code_intoken_map is not None:
            ...
            code_dataflow = code_dataflow[:,0:lens,0:lens]
            ...
            local_mask_map = torch.stack([code_intoken_map for i in range(heads_type[0])]+
					 ...
					 ## [code_AST_map for i in range(heads_type[n])] + , e.g.,  n = 4 
            ...
            '''if len(heads_type)==3:
                ...
                scores = scores + local_mask_map
            else:
                local_mask_map = torch.stack([code_intoken_map for i in range(heads_type[0])]+
                                             [zero_map for i in range(heads_type[1])]+
                                             [code_instatement_map for i in range(heads_type[2])]+
                                             [zero_map for i in range(heads_type[3])]+
                                             [zero_map for i in range(heads_type[4])],1)
                global_enhence_map = torch.stack([zero_map for i in range(heads_type[0])]+
                                             [code_dataflow for i in range(heads_type[1])]+
                                             [zero_map for i in range(heads_type[2])]+
                                             [code_dataflow for i in range(heads_type[3])]+
                                             [zero_map for i in range(heads_type[4])],1)
                global_enhence_map = 5*global_enhence_map.mul(scores)
                scores = scores + local_mask_map + global_enhence_map'''
